...
def get_api_list():
    ...
    url = "http://10.113.248.211:3000/api/interface/list?page=1&limit=1000&project_id=13&status=done,design,undone,testing,deprecated,stoping"
...
def write_api_obj(api_list):
    """yapi中获取的接口数据写入脚本中"""
    try:
        with open(os.path.dirname(os.path.abspath(__file__)) + "\\" + "api_obj.py", "w", encoding='utf-8') as fp:
            # 写顶部
            ...
            for i in range(len(api_list)):
                ...
                urlPath = api_list[i][0].replace("/", "_")
                urlPath = urlPath.replace("_app-http_", "")
                urlPath = urlPath.replace("_back-http_", "")
