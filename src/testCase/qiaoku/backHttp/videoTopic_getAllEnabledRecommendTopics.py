...
class videoTopic_getAllEnabledRecommendTopics(unittest.TestCase):
    """获得所有开启状态的推荐话题列表"""
    #初始化
    def setUp(self):
        # 初始化（清理数据库所有的表）
    ...
    def a(self):
        ...
        backHttp.getAllEnabledRecommendTopics(data,self.token)

    def b(self):
        """不带token"""
        data = {
        }
        backHttp.getAllEnabledRecommendTopics(data)
